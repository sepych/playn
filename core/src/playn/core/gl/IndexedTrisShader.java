...
public class IndexedTrisShader extends GLShader {
	...
	protected class ITCore extends Core {
		...
		protected float[] vertData;
		protected int vertIndex;

		protected short[] elemData;
		...
		public ITCore(String vertShader, String fragShader) {
			...
			vertData = new float[START_VERTS*vertexSize()];
			vertIndex = 0;

			elemData = new short[START_ELEMS];
			elemIndex = 0;
		}
		...
		public void flush() {
			...
			vertices.add(vertData, 0, vertIndex);
			vertIndex = 0;
			elements.add(elemData, 0, elemIndex);
			elemIndex = 0;
		...
		public void addQuad(float m00, float m01, float m10, float m11, float tx, float ty,
			...
			addVertices(m00, m01, m10, m11, tx, ty, x1, y1, sx1, sy1);
			addVertices(m00, m01, m10, m11, tx, ty, x2, y2, sx2, sy2);
			addVertices(m00, m01, m10, m11, tx, ty, x3, y3, sx3, sy3);
			addVertices(m00, m01, m10, m11, tx, ty, x4, y4, sx4, sy4);
		...
		public void addTriangles(float m00, float m01, float m10, float m11, float tx, float ty,
			...
			for (int ii = 0, ll = xys.length; ii < ll; ii += 2) {
				...
				addVertices(m00, m01, m10, m11, tx, ty, x, y, x/tw, y/th);
			}

			for (int ii = 0, ll = indices.length; ii < ll; ii++)
				elemData[elemIndex++] = (short)(vertIdx+indices[ii]);
		...
		public void addTriangles(float m00, float m01, float m10, float m11, float tx, float ty,
			...
			int vertIdx = beginPrimitive(xys.length/2, indices.length);
			for (int ii = 0, ll = xys.length; ii < ll; ii += 2) {
				addVertices(m00, m01, m10, m11, tx, ty, xys[ii], xys[ii+1], sxys[ii], sxys[ii+1]);
			}
		...
		protected void addVertices(float m00, float m01, float m10, float m11, float tx, float ty,
				float x, float y, float sx, float sy){
			vertData[vertIndex++] = m00;
			vertData[vertIndex++] = m01;
			vertData[vertIndex++] = m10;
			vertData[vertIndex++] = m11;
			vertData[vertIndex++] = tx;
			vertData[vertIndex++] = ty;
			vertData[vertIndex++] = arTint;
			vertData[vertIndex++] = gbTint;
		...
		protected int beginPrimitive(int vertexCount, int elemCount) {
			...
			int vertIdx = vertIndex / vertexSize();

			int verts = vertIdx + vertexCount;
			//					int elems = elements.position() + elemCount;
			int elems = elemIndex + elemCount;
		...
		private void expandVerts(int vertCount) {
			...
			vertData = new float[newVerts*vertexSize()];
			vertIndex = 0;
		}
